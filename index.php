...
<body class="bg-dt-base text-dt-text font-sans min-h-screen">
    ...
    <nav class="sticky top-0 z-50 flex items-center gap-2.5 px-4 h-14 bg-dt-surface border-b border-dt-border"
        ...
        <a href="index.php"
            class="text-dt-accent font-bold text-sm flex items-center gap-1.5 no-underline whitespace-nowrap mr-1">
            ⬡ <span>DevTrack</span>
        </a>

        <div class="h-4 w-px bg-dt-border"></div>

        <input id="search-input" type="text" placeholder="Buscar..." class="dt-input max-w-[220px] py-1.5 text-xs">
        ...
        <div class="flex bg-dt-base border border-dt-border rounded-lg overflow-hidden text-xs">
            <button id="btn-view-board" onclick="setView('board')"
                class="px-3 py-1.5 font-medium cursor-pointer border-0 bg-dt-card text-dt-accent transition-colors">⊞
                Board</button>
            <button id="btn-view-table" onclick="setView('table')"
                class="px-3 py-1.5 font-medium cursor-pointer border-0 bg-transparent text-dt-muted hover:text-dt-text transition-colors">☰
                Tabela</button>
        </div>
    ...
    <div id="board-view" class="flex gap-4 p-5 overflow-x-auto min-h-[calc(100vh-56px)] items-start">
        ...
        foreach ($cols as $col): ?>
            <div
                class="flex flex-col min-w-[280px] max-w-[280px] bg-dt-surface border border-dt-border rounded-xl self-start">
                <div
                    class="flex items-center gap-2 px-3 py-2.5 border-b border-dt-border flex-shrink-0 bg-dt-surface rounded-t-xl">
                    ...
                    <span
                        class="col-count text-xs font-semibold text-dt-muted bg-dt-base border border-dt-border rounded-full px-2 py-0.5 tabular-nums"
                        data-col="<?= $col['id'] ?>">0</span>
                    <span class="wip-badge hidden text-[10px] font-bold px-1.5 py-0.5 rounded-full border"
                        data-col="<?= $col['id'] ?>"></span>
    ...
    <div id="modal-backdrop"
        class="fixed inset-0 z-[100] hidden items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
        <div
            class="modal-box-inner bg-dt-surface border border-dt-border/80 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden shadow-modal">
            ...
            <div class="flex items-center gap-3 px-4 py-3 border-b border-dt-border flex-shrink-0">
                ...
                <button id="btn-delete-card" onclick="deleteCard()" style="display:none" class="dt-btn-danger">🗑
                    Excluir</button>
                <button onclick="closeModal()"
                    class="w-6 h-6 flex items-center justify-center text-dt-muted hover:text-dt-text hover:bg-dt-border/60 rounded-lg cursor-pointer bg-transparent border-0 transition-colors text-sm">✕</button>
            </div>

            <div class="flex border-b border-dt-border flex-shrink-0 px-1">
                <button
                    class="modal-tab px-3 py-2 text-xs font-medium border-b-2 border-dt-accent text-dt-accent cursor-pointer bg-transparent border-x-0 border-t-0 transition-colors"
                    data-tab="detalhes" onclick="switchTab('detalhes')">Detalhes</button>
                <button
                    class="modal-tab px-3 py-2 text-xs font-medium border-b-2 border-transparent text-dt-muted hover:text-dt-text cursor-pointer bg-transparent border-x-0 border-t-0 transition-colors"
                    data-tab="historico" onclick="switchTab('historico')">Histórico</button>
            </div>
                ...
                <div class="tab-panel flex flex-col gap-2" id="tab-detalhes">
                    ...
                    <div class="grid grid-cols-3 gap-x-3 gap-y-2">
                        <div>
                            ...
                            <div class="flex gap-1.5">
                                ...
                                <button onclick="toggleInline('new-tipo-section','new-tipo-nome')"
                                    class="dt-btn-icon flex-shrink-0">＋</button>
                            </div>
                            <div id="new-tipo-section" class="hidden mt-2 flex gap-1.5">
                                <input id="new-tipo-nome" type="text" class="dt-input flex-1 !py-1.5 !text-xs"
                                    placeholder="Nome">
                    ...
                    <div class="grid grid-cols-2 gap-x-3 gap-y-2">
                        <div>
                            ...
                            <div class="flex gap-1.5">
                                <select id="f-projeto" class="dt-select sel flex-1">
                                    <option value="">— Sem projeto —</option>
                                </select>
                                <button onclick="toggleInline('new-proj-section','new-proj-nome')"
                                    class="dt-btn-icon flex-shrink-0">＋</button>
                            </div>
                            <div id="new-proj-section" class="hidden mt-2 flex gap-1.5">
                                <input id="new-proj-nome" type="text" class="dt-input flex-1 !py-1.5 !text-xs"
                                    placeholder="Nome">
                    ...
                    <div>
                        ...
                        <textarea id="f-descricao" rows="3" class="dt-input resize-y min-h-[72px]"
                            placeholder="Contexto e o que foi solicitado..."></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="f-solucao-preset" class="dt-select sel mb-2" onchange="onSolucaoPreset(this.value)">
                            ...
                            <optgroup label="Conclusão">
                                ...
                                <option value="Funcionalidade implementada e testada.">Funcionalidade implementada e
                                    testada</option>
                                <option value="Tarefa concluída conforme solicitado.">Tarefa concluída conforme
                                    solicitado</option>
                                <option value="Deploy realizado com sucesso em produção.">Deploy realizado em produção
                                </option>
                                <option value="Correção aplicada e validada em ambiente de produção.">Correção aplicada
                                    e validada</option>
                            </optgroup>
                            <optgroup label="Desenvolvimento">
                                <option value="Código refatorado para melhorar legibilidade e manutenção.">Código
                                    refatorado</option>
                                <option value="Nova API desenvolvida e documentada.">Nova API desenvolvida e documentada
                                </option>
                                <option value="Integração com serviço externo implementada.">Integração com serviço
                                    externo</option>
                                <option value="Otimização de consulta ao banco de dados realizada.">Otimização de
                                    consulta ao banco</option>
                                <option value="Testes automatizados criados e passando.">Testes automatizados criados
                                </option>
                            </optgroup>
                            <optgroup label="Correção">
                                ...
                                <option value="Problema de performance identificado e resolvido.">Problema de
                                    performance resolvido</option>
                                <option value="Falha de segurança corrigida.">Falha de segurança corrigida</option>
                            </optgroup>
                            <optgroup label="Comunicação">
                                <option value="Reunião realizada e alinhamentos documentados.">Reunião realizada e
                                    alinhamentos documentados</option>
                                <option value="Documentação atualizada.">Documentação atualizada</option>
                                <option value="Revisão de código realizada e aprovada.">Revisão de código aprovada
                                </option>
                            </optgroup>
                            <option value="__custom__">✏️ Adicionar comentário personalizado...</option>
                        </select>
                        <textarea id="f-solucao" rows="4" class="dt-input resize-y min-h-[88px] mono hidden"
                            placeholder="Descreva a solução técnica aplicada..."></textarea>
                    </div>

                    <div>
                        <label class="dt-label flex items-center gap-1.5">
                            Tags
                            <button onclick="toggleInline('new-tag-section','new-tag-nome')"
                                class="w-4 h-4 flex items-center justify-center bg-dt-base border border-dt-border rounded text-dt-muted text-xs leading-none cursor-pointer hover:text-dt-text hover:border-dt-muted transition-colors font-normal">＋</button>
                        </label>
                        <input type="hidden" id="f-tags">
                        <input type="hidden" id="f-link">
                        <div id="tag-picker"
                            class="tag-wrap flex flex-wrap gap-1 p-2 bg-dt-base border border-dt-border rounded-lg min-h-[32px] transition-all">
                        </div>
                        <div id="new-tag-section" class="hidden mt-1.5 flex gap-1.5">
                            <input id="new-tag-nome" type="text" class="dt-input flex-1 !py-1 !text-xs"
                    ...
                    <div>
                        <label class="dt-label flex items-center gap-1.5">
                            🔔 Alarme
                            <span class="text-[10px] text-dt-muted/60 font-normal">(notificação no browser)</span>
                        </label>
                        <div class="flex gap-2 items-center">
                            <input id="f-alarme" type="datetime-local" class="dt-input flex-1">
                            <button type="button" id="btn-alarme-limpar" onclick="limparAlarme()" style="display:none"
                    ...
                    <div id="etapas-section" class="hidden">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <span id="etapas-count"
                                class="text-[10px] font-semibold text-dt-muted bg-dt-base border border-dt-border rounded-full px-1.5 py-0.5 tabular-nums">0</span>
                        </div>
                        <div id="etapas-list" class="flex flex-col gap-1.5 mb-2"></div>
                        <div class="flex gap-1.5">
                            <input id="nova-etapa-texto" type="text" class="dt-input flex-1"
                                placeholder="Descreva a etapa atual..."
                                onkeydown="if(event.key==='Enter'){event.preventDefault();adicionarEtapa()}">
                            <button onclick="adicionarEtapa()" class="dt-btn dt-btn-sm whitespace-nowrap">+
                                Etapa</button>
                        </div>
                    </div>
            ...
            <div
                class="flex items-center justify-between px-4 py-2.5 border-t border-dt-border flex-shrink-0 bg-dt-base/30">
